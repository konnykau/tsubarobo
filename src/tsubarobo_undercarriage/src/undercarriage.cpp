...
#include "tsubarobo_undercarriage/msg/velocity_vector.hpp"
...
public:
  Undercarriage_Node()
  : Node("tsubarobo_undercarriage"),tsubarobo_undercarriage_(undercarriage()),tsubarobo_launcher(launcher())
  {
    subscription_ = this->create_subscription<sensor_msgs::msg::Joy>(
      "joy", 10, std::bind(&Undercarriage_Node::topic_callback, this, _1));
    auto_moving_mode_vec_ = this->create_subscription<tsubarobo_undercarriage::msg::VelocityVector>(
      "auto_moving_mode_vec", 10, std::bind(&Undercarriage_Node::auto_moving_mode_callback, this, _1));
...
private:
  ...
  {  
    if(msg.buttons[7]){//startボタン
      tsubarobo_undercarriage_.make_mode(motor_mode::velocity);
    ...
    if(msg.buttons[6]){//backボタン
      tsubarobo_undercarriage_.make_mode(motor_mode::disable);
...
///////////////////////////////ここの下から平行移動、回転をするための個々のモーターのターゲットを決めるif文
    ...
    if(auto_moving_mode_){
      ...
      if(msg.axes[2] == -1){//ZL(left shouldderボタン)
        this->tsubarobo_undercarriage_.update(-msg.axes[0],msg.axes[1],turn_direction::left_turn,dt);
      }//left turn
      else if(msg.axes[5] == -1){//ZR(right shouldderボタン)
          this->tsubarobo_undercarriage_.update(-msg.axes[0],msg.axes[1],turn_direction::right_turn,dt);
      }//right turn
      else{
          this->tsubarobo_undercarriage_.update(-msg.axes[0],msg.axes[1],turn_direction::no_turn,dt);
      }//平行移動//x軸はjoyの入力時点で反転していた

    robomas_pub_1->publish(this->tsubarobo_undercarriage_.make_robomas_Frame());
    ...
    void auto_moving_mode_callback(const tsubarobo_undercarriage::msg::VelocityVector & msg)
    {
      if(auto_moving_mode_){
        auto current_time = this->now();
        double dt = (current_time - last_update_time_).seconds();  // 前回からの経過時間
        last_update_time_ = current_time;
        if(msg.turn == 1){
          this->tsubarobo_undercarriage_.update(msg.x,msg.y,turn_direction::left_turn,dt);
        }
        else if(msg.turn == 2){
          this->tsubarobo_undercarriage_.update(msg.x,msg.y,turn_direction::right_turn,dt);
        }
        else{
          this->tsubarobo_undercarriage_.update(msg.x,msg.y,turn_direction::no_turn,dt);
        }
        robomas_pub_1->publish(this->tsubarobo_undercarriage_.make_robomas_Frame());
      }
    }
  ...
  undercarriage tsubarobo_undercarriage_;
  ...
  rclcpp::Subscription<tsubarobo_undercarriage::msg::VelocityVector>::SharedPtr auto_moving_mode_vec_;
